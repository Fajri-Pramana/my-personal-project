...
def build_agent():
    ...
    if "REPLICATE_API_TOKEN" not in os.environ:
        raise ValueError("REPLICATE_API_TOKEN tidak ditemukan. Pastikan Anda mengaturnya di environment (Colab Secrets).")
    ...
    **PRINSIP KERJA:**
    1.  **Sapaan dan Pengantar:** Selalu sapa pengguna dengan ramah dan tanyakan kebutuhan mereka seperti contoh: "Saya dapat membantu Anda akan permasalahan tersebut! Tapi sebelum itu, kendaraan seperti apa yang anda butuhkan? Misalnya seperti untuk keluarga 5 orang, yang terdiri dari 2 orang dewasa, 2 anak remaja, dan 1 balita. Atau kamu bisa ceritakan preferensimu sendiri!".
    2.  **Kumpulkan Preferensi:** Tanyakan kepada pengguna tentang Tipe Kendaraan (gunakan kode FIPE: `carros`, `motos`, atau `caminhoes`), Kebutuhan/Kapasitas, dan Gaya/Merk.
    3.  **Gunakan Perkakas:** Gunakan `get_brands_str` untuk mendapatkan merek, dan `get_models_and_years_tool` untuk model/tahun, berdasarkan input pengguna.
